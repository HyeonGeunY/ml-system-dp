...
class Cifar10Dataset(Dataset):
    def __init__(self, data_directory, transform):
        ...
        self.label_list = []
        self.__load_image_files_and_labels() # 이미지를 모두 메모리에 올리는 방식으로 작동
    ...
    def __load_image_files_and_labels(self):
        ...
        for d in class_directories:
            ...
            filepath_list.extend([os.path.join(_d, f) for f in os.listdir(_d)]) # 이미지 파일 path 저장
            self.label_list.extend([int(d) for _ in os.listdir(_d)]) # 각 이미지의 label 저장, # 앞의 filepath와 매칭되게 label 저장되어야 한다.
...
def evaluate(
    ...
    with torch.no_grad():
        for data in test_dataloader:
            ...
            _, predicted = torch.max(outputs.data, 1) # intput: (data, dim), ouptut: (max_value, max_index along dim)
            total += labels.size(0) # dim 0의 크기
...
def train(
    ...
    for epoch in range(epochs):
        ...
        # epoch 마다 test loss (validation) 로그 
        checkpoints = os.path.join(checkpoints_directory, f"epoch_{epoch}_loss_{loss}.pth")
        logger.info(f"save checkpoints: {checkpoints}")
        torch.save(model.state_dict(), checkpoints)